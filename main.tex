...
\cref{def:atom} is exactly the \emph{assignment matrix} (confusion matrix) that
readout-error mitigation calibrates and inverts \citep{nation2021m3,bravyi2021mitigating}.
Writing it in the column-stochastic convention the mitigation literature
prefers, with the observed distribution as a column vector acted on from the
left, it is
\begin{equation}
\label{eq:assignment}
  A_i \;=\;
...
assignment matrix is the Kronecker product $A=\bigotimes_i A_i$, which is
\cref{eq:kron} in the column convention: the tensored (local) assignment matrix
used in practice. The element-wise independence that makes $A$ factor is the same
...
parity mode. This is the same Walsh-spectrum object the classical
...
with the per-bit eigenvalue $1-2q_i$ replaced by the latent-conditioned
$1-2r_i$, $r_i\in\{\eps_i,\omega_i\}$.
