...
\begin{abstract}
Quantum readout (measurement) error is, on the density-matrix diagonal, a
classical bit-flip channel: a measured qubit is a truth value
...
``about $n$ times the per-measurement error'' rule of thumb, an overestimate of
tens of percent at modest sizes; detector-error-model tooling composes such
parities exactly but numerically, per circuit, and with a single symmetric rate
per mechanism. The contribution is \emph{forward} prediction
and error budgeting, complementary to inverse mitigation such as M3; the closed
forms hold under per-bit channel independence, with an exact dependence
expansion whose first-order term quantifies weak crosstalk. We position the
calculus against effectus theory, the noisy-Boolean-formula tradition, and
...
\citep{gidney2021stim}. There is at
...
asymmetrically through an entire Boolean tree; an exact expansion in the
dependence structure extends the parity rule beyond independence, its
first-order term quantifying weak readout crosstalk (\cref{sec:crosstalk}). The calculus is precisely the
error model of \emph{Bernoulli type theory}, the author's framework of random
approximate sets, maps, and relations, whose special cases include the classical
Bloom filter \citep{bloom1970} alongside perfect hash filters and
cipher maps \citep{bernoulliSets,bernoulliComposition,bernoulliMaps}.
...
independence and first-order under weak crosstalk.
...
on a specific derived bit, symbolically, before data. On the design-time side,
...
symmetrically; the present calculus is its symbolic, asymmetric counterpart.
...
\citet{vonneumann1956} and \citet{pippenger1988}. That literature owns the parity formula, but
...
that the folklore ``$n$ times the per-measurement error'' rule overestimates by
...
and first-order beyond it (\cref{sec:crosstalk}); and faithful only on the
readout sector. Its value is that it is \emph{reusable} and \emph{symbolic}, the same
