...
\date{June 2026\\[4pt]\small Draft, for review}
...
noisy-Boolean-formula identities, and asymmetric quantum hypothesis testing.
\end{abstract}
...
$\ket{1}\!\to\!\ket{0}$ \citep{tannu2019mitigating,hicks2021readout}. In our
...
\section{The forward $(\eps,\omega)$ calculus}
\label{sec:calculus}
...
assume the channels are mutually independent. The reader should read $\eps$ as
``error when the true answer is $0$ (a false alarm)'' and $\omega$ as ``error
when the true answer is $1$ (a miss)'' uniformly across all derived bits.
...
Theorem~\ref{thm:parity} is classical, it is the noisy-formula / Fourier-analytic
parity identity used by \citet{vonneumann1956} and \citet{pippenger1988} and
throughout noisy-Boolean-formula analysis. We now state its asymmetric,
...
channel, and consider the symmetric per-measurement rate $q_i=q$ for clarity
(\cref{cor:parity-asym} handles the asymmetric and inhomogeneous case verbatim).
...
\emph{numerical}, \emph{per-circuit} procedure operating on measured data. Our
...
The parity identity~\eqref{eq:parity} and the reliability of noisy Boolean
formulas are classical \citep{vonneumann1956,pippenger1988}, and the analysis of
...
\citet{pucha2021certification}. This is the nearest neighbor of our atom
...
\begin{enumerate}[leftmargin=1.6em]
  ...
  \item \textbf{Independence is assumed.} Every closed form above holds under
        per-bit channel independence, equivalently, the tensored assignment
        matrix~\eqref{eq:kron}. Real devices exhibit \emph{measurement crosstalk}:
        the readout of one qubit is correlated with the state or readout of its
        neighbors, which is exactly why correlated models (e.g. the CTMP regime)
        exist. % TODO: confirm the precise scope/claims and a stable primary
        % citation for CTMP-style correlated readout (Bravyi et al. discuss a
        % continuous-time Markov process model; verify before citing by name).
        A correlated-channel extension, propagating a joint, non-factorized
        readout model through the same Boolean operations, is future work; the
        present calculus is the independent/tensor regime.
  ...
  \item \textbf{Empirical validation.} This is a methods/perspective paper; the
        closed forms are derived and the syndrome table is computed exactly from
        them. % TODO: a hardware or simulator study comparing predicted derived-bit
        % rates against measured ones (and quantifying the independence-violation
        % gap from crosstalk) would strengthen the claims but is out of scope for
        % this draft.
